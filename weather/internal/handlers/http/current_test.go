...
import (
	...
	handlers "github.com/GenesisEducationKyiv/software-engineering-school-5-0-velosypedno/weather/internal/handlers/http"
...
func TestWeatherHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := gin.New()
			router.GET("/weather", handlers.NewWeatherGETHandler(mockRepo, requestTimeout))
...
func TestWeatherHandler_Timeout(t *testing.T) {
	// Arrange
	router := gin.New()
	router.GET("/weather", handlers.NewWeatherGETHandler(&timeoutErrRepo{}, time.Millisecond))
